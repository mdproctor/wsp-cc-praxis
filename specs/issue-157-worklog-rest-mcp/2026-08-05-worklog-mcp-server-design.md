...
- `id`, `timestamp`, `event_type`, `work_item_id`, `slot_id`, `repo_path`, `metadata`
...
**Returns:** List of event dicts (same shape as `worklog_events`)
...
def worklog_events(
    ...
    conn = worklog.connect()
    try:
        return worklog.event_log(conn, since=since, event_type=event_type,
...
@mcp.tool()
def worklog_timeline(branch: str, repo_path: str) -> list:
    """Return all events for a branch, oldest first."""
    conn = worklog.connect()
    try:
        return worklog.work_item_timeline(conn, branch, repo_path)
    finally:
        conn.close()


@mcp.tool()
def worklog_slots(family_root: str = None) -> list:
...
1. Creates a temp SQLite database via `worklog.connect(db_path)`
2. Populates with test data via `worklog.record_*()` functions
3. Calls the tool function (with `WORKLOG_DB` env var set to temp path)
4. Asserts return shape and values
